...
export default function People() {
  ...
  async function createReviewer(e: React.FormEvent) {
    ...
    try {
      await api("/users", {
        method: "POST",
        body: JSON.stringify({ email, password, role: "reviewer" }),
      });
  ...
  return (
    <main className="py-9 pb-16 max-w-[1100px]">
      ...
      <p className="text-[var(--ink-soft)] mt-1.5 mb-7">
        Reviewer accounts for this desk. There is no open signup: you create every
        account here.
      </p>
      ...
      {note && (
        <p className="text-[14px] text-[var(--accent-deep)] mb-4">{note}</p>
      )}
      ...
      {!people ? (
        ...
        <div className="border-t border-[var(--line)]">
          {people.map((p) => (
            ...
            >
              <span className="font-semibold text-[14px]">{p.email}</span>
