...
export default function DeskLayout({
    ...
    return (
      <main className="min-h-[100dvh] bg-[var(--parchment)] flex items-center justify-center">
        <div className="text-center">
          <div
            className="text-[28px] font-extrabold animate-pulse"
  ...
  const linkCls = (on: boolean) =>
    `text-[14px] font-medium pb-1 border-b-2 transition-colors ${
      on
    ...
    <div className="min-h-[100dvh] bg-[var(--parchment)]">
      <div className="max-w-[1760px] mx-auto px-8">
        <header className="flex items-center gap-7 py-5 border-b border-[var(--line)]">
          ...
          <div className="ml-auto flex items-center gap-3 text-[14px] text-[var(--ink-soft)]">
            <span>{user.email}</span>
            <span
              className="w-7 h-7 rounded-full bg-[var(--accent)] text-white grid place-items-center text-[14px] font-bold"
              style={{ fontFamily: "var(--font-cabinet)" }}
            >
            ...
            <button
              onClick={signOut}
              className="text-[14px] underline underline-offset-4 hover:text-[var(--accent)]"
            >
              Sign out
            </button>
          </div>
