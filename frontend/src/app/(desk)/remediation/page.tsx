  ...
  return (
    <div className="mt-2 pt-2 border-t border-[var(--line)]">
      <button
        ...
        className={`font-array text-[15px] tracking-wider hover:underline ${
        ...
        <div className="mt-2">
          {files.map((e) => (
            <a
              ...
              className="block font-array text-[15px] text-[var(--accent)] hover:underline truncate"
          ...
          <button
            ...
            disabled={busy}
            className="font-array text-[15px] tracking-wider text-[var(--accent)] hover:underline mt-1 disabled:opacity-50"
          >
            {busy ? "UPLOADING…" : "+ ATTACH PROOF"}
          </button>
          {err && <p className="font-array text-[15px] text-[var(--rust)] mt-1 leading-snug">{err}</p>}
        </div>
      )}
    </div>
  );
  ...
  return (
    ...
    >
      ...
      >
        ...
        <p className="text-[15px] font-semibold mt-2 leading-snug">{f.plain}</p>
        <p className="font-array text-[15px] text-[var(--ink-soft)] mt-1">
      ...
      <Link
        ...
        className="font-array text-[15px] text-[var(--accent)] hover:underline mt-1 inline-block"
      >
      ...
      <div className="flex items-center gap-2 mt-3 pt-2 border-t border-[var(--line)]">
        ...
        <select
          ...
          className="bg-transparent font-array text-[15px] text-[var(--accent)] outline-none max-w-[180px] truncate"
        >
        ...
        <input
          ...
          className={`ml-auto bg-transparent font-array text-[15px] outline-none ${
...
function Column({
  ...
  return (
    ...
    >
      <div className="flex justify-between font-array text-[15px] tracking-[0.09em] text-[var(--ink-soft)] mb-3">
        <span>{col.label}</span>
      ...
      {findings.length === 0 && (
        <p className="font-array text-[15px] text-[var(--ink-soft)] opacity-50 py-4 text-center">
          nothing here
        </p>
      )}
    </div>
  ...
  return (
    <main className="py-9">
      <div className="flex items-baseline gap-4">
        <h1 className="text-[32px] font-bold" style={{ fontFamily: "var(--font-cabinet)" }}>
          Remediation
        </h1>
        {board && (
      ...
      <p className="text-[15px] text-[var(--ink-soft)] mt-2 max-w-[70ch]">
      ...
      <div className="flex items-center gap-2 mt-5 flex-wrap">
        {SCOPES.map((s) => (
          <button
            ...
            className={`font-array text-[15px] tracking-wider px-3 py-[6px] rounded-[3px] border transition-colors ${
              scope === s.key
        ...
        {teams.length > 0 && (
          <select
            ...
            className="font-array text-[15px] tracking-wider bg-transparent border border-[var(--line)] rounded-[3px] px-3 py-[6px] outline-none text-[var(--ink-soft)]"
          >
      ...
      {error && (
        <p className="attn-note mt-4 text-[15px]" role="alert">
          {error}
        </p>
      )}
      ...
      {!board ? (
        ...
        <div className="panel mt-5 overflow-hidden">
          <div className="grid grid-cols-[110px_minmax(0,1fr)_180px_150px_110px_100px] gap-4 px-4 py-3 font-array text-[15px] tracking-[0.09em] text-[var(--ink-soft)] border-b border-[var(--line)]">
            <span>SEVERITY</span>
            <span>FINDING</span>
            <span>ASSET</span>
            <span>OWNER</span>
          ...
          {board.findings.map((f) => (
            ...
            >
              ...
              <span>
                <span className="block text-[15px] font-semibold">{f.plain}</span>
                <span className="block font-array text-[15px] text-[var(--ink-soft)] mt-[2px]">
                  {f.control_id} · {f.inspector}
              ...
              <Link
                href={`/assets/${f.asset_id}`}
                className="font-array text-[15px] text-[var(--accent)] hover:underline truncate"
              >
                {f.asset_name ?? f.asset_id}
              </Link>
              <span className="font-array text-[15px] text-[var(--ink-soft)] truncate">
                {f.owner ?? "unassigned"}
              </span>
              <span
                className={`font-array text-[15px] ${f.overdue ? "text-[var(--rust)] font-bold" : "text-[var(--ink-soft)]"}`}
              >
                {dueLabel(f)}
              </span>
              <span className="font-array text-[15px] tracking-wider text-[var(--ink-soft)]">
                {f.status.replace("_", " ").toUpperCase()}
              </span>
            </div>
          ))}
          {board.findings.length === 0 && (
            <p className="text-[var(--ink-soft)] p-6 text-[15px]">
